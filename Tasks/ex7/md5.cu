 ...
 void WriteArray(char*arr,int arrsize)
{
 ...
 void WriteArrayHex(uint8_t*result,int arrsize)
{
	for (int i = 0; i < arrsize; i++)
	{	
...
__global__ void GenerateSerie(char *characters,unsigned int charsetlen,int passlength,char * strings)
{  
    ...
    for(unsigned long long i = bindex;i<=eindex;i++)
    {
	/*	
		for(int j =0;j<passlength;j++)
		{
			printf("%c",strings[i*passlength+j]);
		}
		printf("\n");
	*/
        gethash(&strings[i],passlength,res);
...
char* GenerateStrings(char *characters,unsigned int charsetlen, int passlength)
{
    unsigned long long beginpassindex = 0;
    unsigned long long	stoppassindex =  (unsigned long long)(pow(charsetlen,passlength));
	//printf("STOP: %llu\n",stoppassindex);
    char*strings = (char*)malloc(passlength*stoppassindex*sizeof(char));
	if(strings == NULL){printf("This array is too big!!!\n");exit(1);}
    ...
    for(unsigned long long i = beginpassindex;i < stoppassindex;i++)
      {
        ...
        //puts("");
		//printf("STOP: %llu:%llu:%lu\n",i,i*passlength,passlength*sizeof(char));
		//printf("STOP: %p:%p\n",strings,strings[i*passlength]);
		
		memcpy(&strings[i*passlength],result,passlength*sizeof(char)); 
		
		//WriteArray(&strings[i*passlength],passlength);
		//puts("");
...
void Compute(char *characters,unsigned int charsetlen, int passlength,unsigned int gridDimx,unsigned int gridDimy,unsigned int blockDimx,unsigned int blockDimy)
{
	...
	//printf("%s",header);
    char*strings = GenerateStrings(characters,charsetlen,passlength);
    //printf("Generated password count %llu\n",totalcount);
    ...
    char * cudastrings;
    cerr = cudaMalloc( &cudastrings, passlength*totalcount * sizeof( char ) );
	if ( cerr != cudaSuccess )
		printf( "CUDA Error [%d] - '%s'\n", __LINE__, cudaGetErrorString( cerr ) );	
    
	cerr = cudaMemcpy( cudastrings, strings, passlength*totalcount * sizeof( char ) , cudaMemcpyHostToDevice );
	if ( cerr != cudaSuccess )
		printf( "CUDA Error [%d] - '%s'\n", __LINE__, cudaGetErrorString( cerr ) );	
    ...
    printf("Generating total of %llu hashes from passwords of length %d from %u characters by %d Threads. Passgen time %f s \n",totalcount,passlength,charsetlen,threadcount,(double)time_used.tv_sec + (double)time_used.tv_usec/1000000);

	gettimeofday(&start, NULL);
	//computation
	GenerateSerie<<< grid_size, block_size >>>(cudachars,charsetlen,passlength,cudastrings);
