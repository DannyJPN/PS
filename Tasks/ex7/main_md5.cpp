...
int main( int num_arg, char **args )
{
	...
	if(num_arg <6)
	{
		printf("Enter password length ,grid sizes and block sizes\n");
		return 1;
		
	}
	
	unsigned int passlength = atoi(args[1]) ;
    //unsigned int threadcount=atoi(args[2]);
    unsigned int gridx=atoi(args[2]);
    unsigned int gridy=atoi(args[3]);
    unsigned int blockx=atoi(args[4]);
    unsigned int blocky=atoi(args[5]);
    ...
    Compute(charset,strlen(charset),passlength,gridx,gridy,blockx,blocky);
