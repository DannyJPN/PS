...
void GenerateSeries(char *characters,unsigned int charsetlen, int len,unsigned long long beginpassindex,unsigned long long stoppassindex)
{
    ...
    char*hashstr=(char*)malloc(32*sizeof(char));
...
int main( int num_arg, char **args )
{
	...
	if(num_arg <3)
	{
		printf("Enter password length and max processes\n");
		return 1;
        ...
        unsigned int passlength = atoi(args[1]) ;
        unsigned int maxpscount=atoi(args[2]);
        //int maxpasslen=3;
        char charset[] = "abcdefghijklmnopqrstwxyz";
		...
		char header[32] = "Processes\tSeconds\n";
		//write(outfile,header,strlen(header));
		printf("%s",header);
		//fclose(outfile);
		//close(outfile);
		unsigned long long totalcount = (unsigned long long)(pow(sizeof(charset)-1,passlength));
		for(processcount=1;processcount <= maxpscount && processcount<=totalcount;processcount++)
		{
		//GenerateSeries(charset,sizeof(charset),passlength,beginpassindex,stoppassindex);
			
			//printf("Generating total of %llu passwords of length %d from %lu characters\nProcesses %d\n",totalcount,passlength,sizeof(charset)-1,processcount);
...
*/		
			...
			{ 
			 struct timeval start, end,time_used;
				...
				{
					//printf( "Child PID %d ended with   status %d\n", wpid,WEXITSTATUS(status) );
				...
				//
				//printf("%llu passwords generated in %lu seconds and %lu microseconds\n",totalcount,time_used.tv_sec , time_used.tv_usec);
				//printf("%llu passwords generated in %f seconds\n",totalcount,(double)time_used.tv_sec + (double)time_used.tv_usec/1000000);
				//char result[256];
				//sprintf(result,"%d\t\t%f",processcount,(double)time_used.tv_sec + (double)time_used.tv_usec/1000000);
				printf("%d\t\t%f",processcount,(double)time_used.tv_sec + (double)time_used.tv_usec/1000000);
				//write(outfile,result,strlen(result));
				//fprintf(outfile,"%d\t\t%f\n",processcount,(double)time_used.tv_sec + (double)time_used.tv_usec/1000000);
				//fclose(outfile);
			...
			//Swap(sharmem,i,i+1);
				...
				if(i == processcount-1){eindex+=remainder;}
				//printf("CHild %d manages %llu - %llu (%llu ks)\n",i,bindex,eindex,eindex-bindex+1);
